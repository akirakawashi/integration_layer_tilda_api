...
`integration_layer_tilda_api` решает одну прикладную задачу: безопасно принять webhook из Tilda, отложенно обработать файл из формы и сохранить его в Nextcloud. Tilda при этом не ждёт долгую загрузку и обработку архива.
...
Такой разрез оставляет приём webhook'ов быстрым и идемпотентным, а тяжёлую работу с файлами уводит в фоновую очередь.
...
- не плодит дубликаты при повторной доставке одного и того же `tranid`;
- worker забирает следующую готовую задачу с lease-lock через БД;
- скачивает только поддерживаемые архивы и проверяет их реальное содержимое;
...
2. API разбирает form payload, при необходимости проверяет API key и достаёт ссылку на файл.
3. Если это test-webhook (`test=test`), сервис сразу отвечает `ok` и не пишет задачу в БД.
...
История переходов отдельно хранится в `tilda_jobs_status_history`.
...
Сервис ждёт `application/x-www-form-urlencoded`, не JSON.
...
Если прямого совпадения нет, сервис берёт первое URL-значение из поля, название которого похоже на upload/file/archive/document.

## Идемпотентность и очередь

Сервис не создаёт повторные задачи на один и тот же `tranid`.

На уровне БД и SQL это выглядит так:
...
С такой схемой сервис переживает повторную доставку webhook'ов. Ещё можно запускать больше одного worker-процесса на одной БД без гонок за одну и ту же задачу.
...
Для `.zip` проверка идёт через `zipfile.is_zipfile`, для `.rar` — через RAR4/RAR5 magic bytes.

Если remote storage возвращает HTML вместо архива, сервис различает два случая: "файл ещё не готов" и "пришла не та страница". В зависимости от причины задача уходит в `retry_wait` или `failed`.
...
- duplicate webhook тоже завершается `ok`;
...
- для более высокого throughput можно запускать несколько worker'ов на одной БД;
